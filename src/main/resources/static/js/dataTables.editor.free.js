   ...
   $.extend( altEditor.prototype, {
       ...
       _setup: function()
       {
         ...
         if( this.s.dt.button('edit:name') )
         {
            ...
            {
                if (this.getAttribute("data-target") == that.s.namespace) {
                    e.preventDefault();
                    e.stopPropagation();
                    that._editRowData();
                }
       ...
       {
         var that = this;
         var dt = this.s.dt, target = this.s.namespace;
         ...
         for( var i = 0; i < dt.context[0].aoColumns.length; i++ )
         {
            columnDefs.push({
                ...
                type: dt.context[0].aoColumns[i]._sManualType,
                dataType: dt.context[0].aoColumns[i].dataType?dt.context[0].aoColumns[i].dataType:null,
                hidden: dt.context[0].aoColumns[i].hidden
            })
         }
          ...
          for (var j in columnDefs) {
            data += "<div class='form-group'>" +
                        "<div class='col-sm-3 col-md-3 col-lg-3 text-right' style='padding-top:7px;'>" +
                            "<label for='" + columnDefs[j].title + "'>" + columnDefs[j].title + ":</label>" +
                        "</div>" +
                        "<div class='col-sm-9 col-md-9 col-lg-9'>" +
                             that._rendEditField(columnDefs[j], rowData, j) +
                        "</div>" +
          ...
          $('#altEditor-modal').on('show.bs.modal', function() {
            $('#altEditor-modal').find('.modal-title').html('Update Step');
            ...
            $('#altEditor-modal').find('.modal-footer').html("<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>Close</button>\
               <button type='button' data-content='remove' class='btn btn-primary' data-target='" + target + "' id='editRowBtn'>Update Step</button>");
          });
           $('#altEditor-modal').modal('show');
          var elementFields = $('#altEditor-modal form[name="altEditor-form"]').find(".form-control"), i;
          for (i = 0; i < elementFields.length; i++) {
              $(elementFields[i]).val(rowData.data()[0][i]);
          }

          $('#altEditor-modal input:first').focus();

       },
       _rendEditField: function (columnDef, rowData, index) {
           var dom, disabledOption = "";
           if (columnDef.hidden === true) {
               disabledOption = "disabled";
           }
           switch (columnDef.type){
               case "dropdown":
                   var options, i;
                   if (columnDef.dataType && columnDef.dataType.length > 0) {
                       for (i = 0; i < columnDef.dataType.length; i++) {
                           options+= "<option value='" + columnDef.dataType[i] + "'>" + columnDef.dataType[i] + "</option>";
                       }
                   }
                   dom = "<select id='" + columnDef.title +
                       "' style='overflow:hidden' " + disabledOption + " class='form-control  form-control-sm'>" +
                       options +
                       "</select>";
                   break;
               case "numeric":
                   dom = "<input type='number' id='" + columnDef.title +
                       "' name='" + columnDef.title + "' placeholder='" + columnDef.title +
                       "' style='overflow:hidden' " + disabledOption + " class='form-control  form-control-sm' min='1'>";
                   break;
               default:
                   dom = "<input type='text'  id='" + columnDef.title +
                       "' name='" + columnDef.title + "' placeholder='" + columnDef.title +
                       "' style='overflow:hidden' " + disabledOption + " class='form-control  form-control-sm'>";
                   break;
           }
       ...
       _openDeleteModal: function ()
       {
         ...
         for( var i = 0; i < dt.context[0].aoColumns.length; i++ )
         {
          ...
          $('#altEditor-modal').on('show.bs.modal', function() {
            $('#altEditor-modal').find('.modal-title').html('Delete Step');
            $('#altEditor-modal').find('.modal-body').html('<pre>' + data + '</pre>');
            $('#altEditor-modal').find('.modal-footer').html("<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>Close</button>\
               <button type='button' data-content='remove' class='btn btn-danger' data-target='" + target + "' id='deleteRowBtn'>Delete</button>");
          });
          $('#altEditor-modal').modal('show');
          $('#altEditor-modal input:first').focus();
         ...
         for( var i = 0; i < dt.context[0].aoColumns.length; i++ )
         {
          ...
          for (var j in columnDefs) {
            data += "<div class='form-group'>" +
                        ...
                        "</div>" +
                        "<div class='col-sm-9 col-md-9 col-lg-9'>" +
                             that._rendAddField(columnDefs[j], count) +
                        "</div>" +
          ...
          $('#altEditor-modal').on('show.bs.modal', function() {
            $('#altEditor-modal').find('.modal-title').html('Add Step');
            $('#altEditor-modal').find('.modal-body').html('<pre>' + data + '</pre>');
            $('#altEditor-modal').find('.modal-footer').html("<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>Close</button>\
               <button type='button' data-content='remove' class='btn btn-primary' data-target='" + target + "' id='addRowBtn'>Add Step</button>");
          });
          $('#altEditor-modal').modal('show');
          $('#altEditor-modal input:first').focus();
       },
       _rendAddField: function (columnDef, count) {
           var dom, disabledOption;
           if (columnDef.hidden === true) {
               disabledOption = "disabled";
           }
           switch (columnDef.type){
               case "dropdown":
                   var options;
                   if (columnDef.dataType && columnDef.dataType.length > 0) {
                       for (var i = 0; i < columnDef.dataType.length; i++) {
                           options+= "<option value='" + columnDef.dataType[i] + "'>" + columnDef.dataType[i] + "</option>";
                       }
                   }
                   dom = "<select id='" + columnDef.title +
               ...
               case "numeric":
                   count++;
                   dom = "<input type='number'  id='" + columnDef.title + "' name='" + columnDef.title + "' placeholder='" +
                       columnDef.title + "' style='overflow:hidden' " + disabledOption + " class='form-control  form-control-sm' value='" + count + "' min='1'>";
                   break;
               default:
                   dom = "<input type='text'  id='" + columnDef.title + "' name='" + columnDef.title + "' placeholder='" +
                       columnDef.title + "' style='overflow:hidden' " + disabledOption + " class='form-control  form-control-sm' value=''>";
                   break;
           }
           return dom;
       },
